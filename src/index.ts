...
app.post("/users", async (req: Request, res: Response) => {
    try {
        ...
        res.status(201).send({ message: "User criado com sucesso", users: newUser })
    } catch (error) {
        console.log(error)
...
app.delete("/users/:id", async (req: Request, res: Response) => {
    try {
        ...
        if (IdToDelete[0] !== "f") {
            res.status(404)
            throw new Error("'id' deve iniciar em 'f'")
        }
...
app.post("/tasks", async (req: Request, res: Response) => {
    try {
        ...
        res.status(201).send({ message: "Task criado com sucesso", users: insertedTask })
    } catch (error) {
        console.log(error)
...
app.put("/tasks/:id", async (req: Request, res: Response) => {
    try {
        ...
        const newCreateAt = req.body.createAt
        ...
        if (newStatus !== undefined) {
            if(typeof newStatus !== "number") {
                res.status(400)
                throw new Error("'status' deve ser número (0 para imcompleta ou para completa 1)")
            }
        }
        ...
        const newTask : TTaskDB = {
            ...
            created_at: newCreateAt || taskToEdit.created_at,
            status: isNaN(newStatus) ? taskToEdit.status : newStatus
        }

        await db("tasks").update(newTask).where({ id: idToEdit })

        res.status(201).send({ message: "Task editado com sucesso", task: newTask })
    } catch (error) {
        console.log(error)
...
app.delete("/tasks/:id", async (req: Request, res: Response) => {
    try {
        ...
        if (IdToDelete[0] !== "t") {
            res.status(404)
            throw new Error("'id' deve iniciar em 't'")
        }
...
//usersTasks POST
app.post("/tasks/:taskId/users/:usersId", async (req: Request, res: Response) => {
    try {
        const taskId = req.params.taskId
        const userId = req.params.usersId
        if (taskId[0] !== "t") {
            res.status(404)
            throw new Error("'taskId' deve iniciar em 't'")
        }
        if (userId[0] !== "t") {
            res.status(404)
            throw new Error("'usersId' deve iniciar em 't'")
        }
        ...
        res.status(201).send({ message: "User atribuido a uma tarefa"})
    } catch (error) {
        console.log(error)
...
//deleteUserTask
app.delete("/tasks/:taskId/users/:usersId", async (req: Request, res: Response) => {
    try {
        const taskIdToDelete = req.params.taskId
        const userIdToDelete = req.params.usersId
        if (taskIdToDelete[0] !== "t") {
            res.status(404)
            throw new Error("'taskId' deve iniciar em 't'")
        }
        if (userIdToDelete[0] !== "t") {
            res.status(404)
            throw new Error("'usersId' deve iniciar em 't'")
        }
